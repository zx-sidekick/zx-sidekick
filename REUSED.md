...
| `app/src/frontend/{mod,video,headless,gamepad}.rs`, `app/src/main.rs` | the earlier build (the frontend ones from starquake-recompiled's `games/starquake/src/frontend/`) | without the guidance panel, its picker and the game tracking behind it at first; #25 brought the panel, picker and tracking back (see the row below) |
| `app/src/frontend/{guidance,track,overlay,panel}.rs`, `app/src/frontend/overlay.wgsl`, the picker's parts of `mod.rs`, `video.rs` and `gamepad.rs`, the drawing added to `text.rs`, and the key hold and watched addresses in `crates/sidekick/src/machine.rs` | the earlier build (the frontend ones from starquake-recompiled's `games/starquake/src/frontend/`) | without the guidance levels' own data and drawing (codes, map, pieces); legends drawn to the rule on #3; the pause notice drawn in the overlay; End this game resumes a paused game first |
| the menu, play, game-over, hand-over and death entry points and the End this game keys in `crates/sidekick/src/starquake.rs`, and `sk-check facts` | the earlier build | only what the framework follows; checked in every control method and from a paused game |
