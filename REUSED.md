...
| `app/src/frontend/{guidance,track,overlay,panel}.rs`, `app/src/frontend/overlay.wgsl`, the picker's parts of `mod.rs`, `video.rs` and `gamepad.rs`, the drawing added to `text.rs`, and the key hold and watched addresses in `crates/sidekick/src/machine.rs` | the earlier build (the frontend ones from starquake-recompiled's `games/starquake/src/frontend/`) | without the map and pieces (#5, #6), and level 1's codes brought back by #4; legends drawn to the rule on #3; the pause notice drawn in the overlay |
| the menu, play, game-over, hand-over and death entry points and the End this game keys in `crates/sidekick/src/starquake.rs`, and `sk-check facts` | the earlier build | only what the framework follows; checked in every control method |
