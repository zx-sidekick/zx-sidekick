...
- **keys**: chooses each of the five control methods on the title screen in turn, starts a game, and drives it with the joystick alone: in every method each direction and fire move the picture where Blob is, Start freezes it and a direction then resumes it, and the control method, its key tables and the pause key read as recorded.
