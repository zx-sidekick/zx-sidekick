...
- [x] Training mode's four switches: time stands still, full gun and platforms, endless lives, no harm from enemies; each written back between frames while it is on, and named on the game's score (built; checked by hand). The kinds of enemy that kill on touch, the deadly patches and the zappers are not stopped by any of them: #68.
