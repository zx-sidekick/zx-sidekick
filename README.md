...
- [x] The guidance panel beside the game, the picker for the guidance level and training mode's switches (Esc, or Select on a gamepad), End this game and Exit, the note of how much help a game had beside the game-over screens, and pausing with Start or the pause key.
- [x] Guidance level 1: the codes you have been shown, in a column down the panel's right in the game's own letters: every teleporter booth you have entered, and the three chips each security door asked for once its screen has shown them.
- [x] The items found marked on the map from level 2, drawn with the game's own graphics at a whole number of screen pixels a game pixel, outlined, and a missing core piece in a room you have seen drawn as itself.
...
- [x] Guidance level 5: the same two routes through the whole map, as the map reads the rooms, dashed through rooms you have not visited. It assumes Blob can fly everywhere and that doors and wall passages work, with lifts only ever going up, and leaves the logistics to the player (built; checked by hand).
- [ ] Training mode's five switches: time stands still, full gun and platforms, endless lives, enemies drain no energy, and the spikes and zappers are harmless; each written back between frames while it is on, and named on the game's score (built; awaiting a check by hand).
...
Beside the picture is the guidance panel. Esc, or Select on a gamepad, opens the picker and holds the game while it is open: up and down choose a row, left and right change it, Enter or A keeps the changes, and Esc, B or Select cancel them. It sets the guidance level (0 to 5) and training mode, and asks before a change that would show on the game's score. At level 1 the panel lists the codes you have been shown this game, in a column at its right in the game's own letters: the teleporter booths you have entered, and the three chips each security door asked for once its screen showed them; at level 2 it adds a map of the rooms you have visited, with their openings, the room you are in and the teleporters you have seen; at level 2 it also marks each item you have found lying in a room, drawn as what the item does, a chip, the card that opens any door, the pad key or something a pyramid trades; at level 3 it marks the rooms holding a core piece still needed, visited or not, and shows the core's nine holes in a row above the map, drawn with the game's own graphics from memory; at level 4 it draws two routes over the ways you have walked, to the nearest missing piece in pink and to the core in orange while you carry a piece it needs, each with an arrow in the picture's border marked "item" or "core" so the two are told apart without colour, a legend of the two chips under the map, and the code to select when a teleporter is next, with Tab, or Y on a gamepad, switching the pink route between the three nearest missing pieces; at level 5 the same routes run through the whole map, dashed through rooms you have not visited, ending in the part of the room a piece is in once you have entered its room, assuming Blob can fly everywhere, doors and wall passages work, and lifts only ever go up. Training mode is five switches of its own, off unless you turn them on: time stands still, so energy drains only on contact; the gun and platform bars stay full; losing a life does not cost one; enemies drain no energy, though the kinds that kill outright still kill; and the spikes and zappers are harmless. Each is held by writing to the game's memory between frames, never by changing the game, and every one used shows on that game's score. End this game abandons the game in play the game's own way, by holding A S D F G, and Exit closes the program; both need a second press. Beside the game-over and high-score screens the panel says how much help the game had.
...
- **training**: 1,200 frames of the same scripted play from one saved start, under each switch alone. With every switch off, energy drains to 3 of 123 and a life is lost; time standing still leaves it at 27 or better; the gun and platform bars end no lower than they started where a plain run spends them; endless lives never falls below where it started, where a plain run does; and time standing still with enemies draining nothing together leaves energy untouched at 127. Standing Blob on the spike or zapper in each of six rooms kills him in 6 of 6; with the spikes-and-zappers switch on, in 0 of 6. With every switch off nothing is written, so the play ends exactly as the game left it.
