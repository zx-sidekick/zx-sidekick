...
- [ ] Guidance level 2: a map of the rooms you have visited, with every edge shown open or closed and walls inside divided rooms (built; awaiting a check by hand). Levels 3 to 5 and training mode are still to come.
...
- **facts**: the entry points the guidance panel follows arrive in order, the menu and then play, and holding A S D F G from the top of the play loop, as End this game does, reaches the game-over screens and comes back to the menu, in all five control methods; the teleporter table holds fifteen five-letter codes in fifteen rooms, and walking into each booth has the game print the code the table gives for its room, 15 of 15.
