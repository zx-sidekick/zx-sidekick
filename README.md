...
- [x] Training mode's four switches: time stands still, full gun and platforms, endless lives, no harm from enemies; each written back between frames while it is on, and named on the game's score (built; checked by hand). No harm from enemies also stops the spinning tops that kill on touch, the impalers and the zap rays, by steering a register at the instruction the game decides each with, so nothing drawn changes (built; checked against the game, not yet played).
