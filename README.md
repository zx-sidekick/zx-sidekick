...
- [ ] Guidance level 5: the same two routes through the whole map, as the map reads the rooms, dashed through rooms you have not visited. Doors stay shut, a lift only goes up, a wall passage leads one way, and a climb with nothing to lift Blob counts only once walked (built; awaiting a check by hand). Training mode is still to come.
