...
2. [ ] **The machine.** `zx-core` (tape, screen, PNG, SHA-1, snapshot, timing with ULA contention) and `zx-spectrum` (a 48K bus around `rustzx-z80` from our fork). The Fuse corpus: 1,329 of 1,335 exact, the 6 undocumented-flag cases listed, bus activity 1,335 of 1,335; in the gate and in CI.
3. [ ] **Starquake with no ROM.** `JR $` traps at the three ROM entries, the answers to MASK-INT, PRINT-A-2 and HL-HL×DE, the start state from the tape, `docs/rom.md`. `sk-check entry` (the real loader returns where the game is started) and `sk-check rom` (every ROM call answered as the real ROM does).
